 ...
 */
LIBSSH2_SESSION *php_ssh2_session_connect(char *host, int port, zval *methods, zval *callbacks, char *bindto)
{
	...
	socket = php_network_connect_socket_to_host(host, port, SOCK_STREAM, 0, &tv, NULL, NULL, bindto, 0, STREAM_SOCKOP_NONE);
...
/* {{{ proto resource ssh2_connect(string host[, int port[, array methods[, array callbacks, string bindto]]]])
...
PHP_FUNCTION(ssh2_connect)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "s|la!a!s!", &host, &host_len, &port, &methods, &callbacks, &bindto, &bindto_len) == FAILURE) {
		return;
	}

	session = php_ssh2_session_connect(host, port, methods, callbacks, bindto);
	if (!session) {
